...
const isBx24Available = (): boolean => {
    const bx24 = (globalThis as any).BX24
    return !!bx24 && typeof bx24.callMethod === 'function'
}

const bitrixWebhookFetch = async (method: string, params: Record<string, unknown> = {}): Promise<any> => {
    ...
    if (!response.ok) {
        throw new Error(`Bitrix24 webhook HTTP ${response.status}: ${response.statusText}`)
    }
...
const normalizeListResult = (result: any): any[] => {
...
const callApiViaWebhook = async (
    ...
    if (method.endsWith('.get')) {
        ...
        return callGetMethod(method, id)
    }

    const maxTotal = 50
    let start = batchNumber * 2500
    const data: any[] = []

    while (true) {
        const result = await bitrixWebhookFetch(method, buildListParams(method, filter, select, entityTypeId, start))
        data.push(...normalizeListResult(result))
        ...
        start = result.next
        if (data.length > 0 && data.length % maxTotal !== 0) {
            break
        }
    }
...
export async function callApi(method: string, filter: {}, select: string[] | null, entityTypeId: number | null, batchNumber: number = 0, parsed: number = 0): Promise<any[]> {
    if (!isBx24Available()) {
        return callApiViaWebhook(method, filter, select, entityTypeId, batchNumber)
    }
...
export async function callGetMethod(method: string, id: number | string): Promise<any> {
    if (!isBx24Available()) {
        return bitrixWebhookFetch(method, { id })
    }
...
export async function callMethod(method: string, id: number | string, fields: Record<string, unknown>, entityTypeId: number | string): Promise<any> {
    if (!isBx24Available()) {
        return bitrixWebhookFetch(method, { id, fields, ...(entityTypeId !== undefined ? { entityTypeId } : {}) })
    }
...
/** Методы REST без пары id/fields на верхнем уровне (например crm.timeline.comment.add). */
export async function callMethodWithParams(method: string, params: Record<string, unknown>): Promise<any> {
    if (!isBx24Available()) {
        return bitrixWebhookFetch(method, params)
    }
